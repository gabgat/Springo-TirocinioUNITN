...
class NSSH:
    def __init__(self, ip, port, tools_dir, timestamp):
        ...
        self.args = f"-sC -sV --script=ssh-* -p {self.port}"
        self.output_file = os.path.join(self.tools_dir, f"nssh_{self.port}_{self.timestamp}.json")
    ...
    def save_json_result(self, raw_output):
        try:
            ...
            printout(f"NSSH JSON results saved to: {self.output_file}")
