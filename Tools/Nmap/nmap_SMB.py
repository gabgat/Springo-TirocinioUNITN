...
class NSMB:
    def __init__(self, ip, port, tools_dir, timestamp):
        ...
        self.args = f"-Pn --script=smb-* -p {self.port}"
        self.output_file = os.path.join(self.tools_dir, f"nsmb_{self.port}_{self.timestamp}.json")
    ...
    def save_json_result(self, raw_output):
        try:
            ...
            printout(f"NSMB JSON results saved to: {self.output_file}")
