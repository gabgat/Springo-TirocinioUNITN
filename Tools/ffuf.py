...
class FFUF:
    def __init__(self, url, port, tools_dir, timestamp, threads):
        ...
        self.output_file = os.path.join(self.tools_dir, f"FFUF_{self.port}_{self.timestamp}.txt")
    ...
    def run_ffuf(self):
        ...
        cmd = f"ffuf -w {self.wordlist}:FUZZ -u {self.url}/FUZZ -recursion -recursion-depth 2 -e {','.join(self.extensions)} -mc {','.join(self.response)} -ac -ic -v -t {self.threads} -o {self.output_file}"
